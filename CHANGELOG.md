...
- `bill`: `Line.BuyerItemRef` and `SubLine.BuyerItemRef` — the reference code assigned by the buyer to identify the item or service supplied on the line (EN 16931 BT-156). The buyer's code is customer–item relationship data, so it lives on the line rather than on the item.
